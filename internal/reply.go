...
🔄 Исправьте синтаксис и попробуйте снова!`, err, aiResponse)
	...
	if json.Unmarshal([]byte(jsResult), &resultObj) == nil {
		...
		outputData, hasOutput := resultObj["output"].(string)
		...
		if hasOutput && outputData != "" {
			log.Printf("🔄 JavaScript returned output data, continuing GPT conversation")

			// Add output data to conversation context and continue with AI
			outputMessage := fmt.Sprintf("JavaScript execution result: %s", outputData)
			...
			// Generate AI response with the new context
			continueResponse, err := aiService.GenerateResponseWithContext(ctx, "Продолжай диалог на основе результатов JavaScript", messages, "")
			if err != nil {
				log.Printf("Error generating continuation response: %v", err)
				return
			}

			// Recursively execute the continuation response as JavaScript
			log.Printf("🔄 RECURSIVE EXECUTION for user %d", user.ID)
