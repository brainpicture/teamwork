...
func NewClaudeProvider(apiKey string) *ClaudeProvider {
	...
	return &ClaudeProvider{
		client: client,
		model:  string(anthropic.Claude3Opus20240229),
	}
}
...
func (p *OpenAIProvider) GenerateResponseWithContextAndProject(ctx context.Context, prompt string, history []*Message, currentProject *Project) (string, error) {
	...
	if currentProject != nil {
		projectInfo := fmt.Sprintf("\n\nТЕКУЩИЙ ПРОЕКТ ПОЛЬЗОВАТЕЛЯ:\n- ID: %d\n- Название: %s\n- Описание: %s\n- Статус: %s\n\nМожешь упоминать этот проект в контексте разговора.",
			currentProject.ID, currentProject.Title, currentProject.Description, currentProject.Status)
	...
	response := resp.Choices[0].Message.Content
	log.Printf("AI Response with project context generated: %d characters", len(response))
	return response, nil
}

// ClaudeProvider implementations
...
// TranscribeAudio - Claude doesn't support audio transcription
func (p *ClaudeProvider) TranscribeAudio(ctx context.Context, audioData io.Reader, filename string) (string, error) {
	return "", fmt.Errorf("audio transcription not supported by Claude provider")
}
...
// GenerateResponseWithContextAndProject generates a response using Claude with conversation history and current project context
func (p *ClaudeProvider) GenerateResponseWithContextAndProject(ctx context.Context, prompt string, history []*Message, currentProject *Project) (string, error) {
	...
	response := resp.Content[0].Text
	log.Printf("Claude Response with project context generated: %d characters", len(response))
	return response, nil
}
...
func (s *AIService) GenerateResponseWithContextAndProject(ctx context.Context, prompt string, history []*Message, currentProject *Project, fallback string) (string, error) {
	...
	response, err := s.provider.GenerateResponseWithContextAndProject(ctx, prompt, history, currentProject)
	if err != nil {
		return "", err
	}
